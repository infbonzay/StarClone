...
//	  if (!(img->flags & SC_IMAGE_FLAG_DISABLEDRAW))
	{
		Play_sfxdata(img->parentimg->xpos>>8,img->parentimg->ypos>>8,sndid,2,0);
	}
	img->offsetcmdinbuf += cmdsize;
	return(0);
}
...
//	  if (!(img->flags & SC_IMAGE_FLAG_DISABLEDRAW))
	{
		...
		Play_sfxdata(img->parentimg->xpos>>8,img->parentimg->ypos>>8,sndid1+myrand(sndid2-sndid1+1),2,0);
	}
	img->offsetcmdinbuf += cmdsize;
	return(0);
}
			...
			switch(buf[0])
			{
				case 1:
					...
					if (weapon_id >= MAX_WEAPONS_ELEM)
					{
						...
						{
							DEBUGMESSCR("SC_Unit=%d iscriptcmd: attackwith %d with no ground weapon\n",a->SC_Unit,buf[0]);
						}
					}
					break;
				case 2:
...
//						}
				...
				default:
					weapon_id = 0;
					DEBUGMESSCR("SC_Unit = %d attack with %d != 1 or 2 \n",a->SC_Unit,buf[0]);
					break;
			}
			if (a->finalOBJ)
			{
