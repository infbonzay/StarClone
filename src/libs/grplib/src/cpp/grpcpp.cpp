...
//==========================================================
inline void invmemset(char *adrmem,char mask,int count)
{
...
void putfog32x32(long x,long y,char *table,long lt,long rt,long lb,long rb)
{
    ...
    {
	adrvid = GRP_vidmem+GRP_scanlineoffsets[y]+x+sx-1;
	fromy=rt<<8;
	toy=lt<<8;
	deltay1=(rb-rt)<<3;
	deltay2=(lb-lt)<<3;
    ...
    {
	adrvid = GRP_vidmem+GRP_scanlineoffsets[y+sy-1]+x;
	fromy=lb<<8;
	toy=rb<<8;
	deltay1=(lt-lb)<<3;
	deltay2=(rt-rb)<<3;
    ...
    {
	adrvid = GRP_vidmem+GRP_scanlineoffsets[y+sy-1]+x+sx-1;
	fromy=rb<<8;
	toy=lb<<8;
	deltay1=(rt-rb)<<3;
	deltay2=(lt-lb)<<3;
