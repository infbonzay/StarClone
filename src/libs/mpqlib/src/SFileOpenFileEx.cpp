...
static BOOL OpenLocalFile(const char * szFileName, HANDLE * phFile)
{
    ...
    if(hFile != INVALID_HANDLE_VALUE)
    {
        ...
        UINT nHandleSize = sizeof(TMPQFile) + strlen(szFileName); 
...
int WINAPI SFileEnumLocales(HANDLE hMPQ, const char * szFileName, UINT * plcLocales, UINT * pdwMaxLocales)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        for(UINT i = 0; i < dwLocales; i++, pHash++)
            *plcLocales++ = (UINT)pHash->lcLocale;
    }
    return nError;
}
...
BOOL WINAPI SFileOpenFileEx(HANDLE hMPQ, const char * szFileName, UINT dwSearchScope, HANDLE * phFile)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        if(dwBlockIndex == (UINT)-1 || dwBlockIndex > ha->pHeader->dwBlockTableSize)
            nError = ERROR_FILE_NOT_FOUND;
    }
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        if(hf->pBlock->dwFlags & MPQ_FILE_COMPRESSED)
        {
            ...
            if((hf->pdwBlockPos = ALLOCMEM(UINT, hf->nBlocks + 1)) == NULL)
                nError = ERROR_NOT_ENOUGH_MEMORY;
        }
