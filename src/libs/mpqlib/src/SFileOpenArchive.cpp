...
static int RelocateMpqTablePositions(TMPQArchive * ha)
{
    ...
    UINT dwFileSize = GetFileSize(ha->hFile, NULL);
...
BOOL SFileOpenArchiveEx(const char * szMpqName, UINT dwPriority, UINT /* dwFlags */, HANDLE * phMPQ, UINT dwAccessMode)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        DecryptHashTable((UINT *)ha->pHashTable, (BYTE *)"(hash table)", (ha->pHeader->dwHashTableSize * 4));
	    ...
	    // UPDATE: This check hits false positives now - removed
            if(0) //(pHash->lcLocale != 0xFFFFFFFF && (pHash->lcLocale & 0x00FF0000) != 0)
            {
                nError = ERROR_BAD_FORMAT;
                break;
            }
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        UINT dwArchiveSize = ha->pHeader->dwArchiveSize + ha->dwMpqPos;

        if(ha->pHeader->dwDataOffs != ha->pBlockTable->dwFilePos)
            DecryptBlockTable((UINT *)ha->pBlockTable, (BYTE *)"(block table)", (ha->pHeader->dwBlockTableSize * 4));
