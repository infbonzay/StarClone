...
UINT          lcLocale   = LANG_NEUTRAL;// File locale
...
TMPQHash * GetHashEntry(TMPQArchive * ha, const char * szFileName)
{
    ...
    UINT dwIndex = (UINT)(long)szFileName;        // Hash index
    UINT dwName1;
    UINT dwName2;
...
int AddFileToArchive(TMPQArchive * ha, HANDLE hFile, const char * szArchivedName, UINT dwFlags, UINT dwQuality, int nFileType, BOOL * pbReplaced)
{
    ...
    UINT * pdwBlockPos = NULL;         // Block position table (compressed files only)
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        UINT dwFilePos = sizeof(TMPQHeader) + ha->dwMpqPos;
    ...
    {
        pdwBlockPos  = ALLOCMEM(UINT, nBlocks);
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        for(nBlock = 0; nBlock < nBlocks-1; nBlock++)
        {
            ...
            if(pBlock->dwFlags & MPQ_FILE_ENCRYPTED)
            {
                CONVERTBUFFERTOLITTLEENDIAN32BITS((UINT *)pbToWrite, dwOutLength / sizeof(UINT));
                EncryptMPQBlock((UINT *)pbToWrite, dwOutLength, dwSeed1 + nBlock);
                CONVERTBUFFERTOLITTLEENDIAN32BITS((UINT *)pbToWrite, dwOutLength / sizeof(UINT));
            }
