...
BOOL WINAPI SFileCreateArchiveEx(const char * szMpqName, UINT dwCreationDisposition, UINT dwHashTableSize, HANDLE * phMPQ)
{
    ...
    bFileExists = access(szMpqName,R_OK)!=-1;
...
BOOL WINAPI SFileAddFileEx(HANDLE hMPQ, const char * szFileName, const char * szArchivedName, UINT dwFlags, UINT dwQuality, int nFileType)
{
...
BOOL WINAPI SFileRemoveFile(HANDLE hMPQ, const char * szFileName)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        if(IsValidMpqHandle(ha) == FALSE)
            nError = ERROR_INVALID_PARAMETER;
        if((long)szFileName > 0x00010000 && *szFileName == 0)
            nError = ERROR_INVALID_PARAMETER;
    }
    ...
    if(nError == ERROR_SUCCESS)
    {
        if((long)szFileName > 0x00010000 && !stricmp(szFileName, LISTFILE_NAME))
