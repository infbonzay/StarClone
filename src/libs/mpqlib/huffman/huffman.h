...
class TInputStream
{
    ...
    unsigned int GetBit();
    unsigned int Get7Bits();
    unsigned int Get8Bits();
 
    unsigned char * pbInBuffer;         // 00 - Input data
    unsigned int   dwBitBuff;          // 04 - Input bit buffer
...
class TOutputStream
{
    ...
    void PutBits(unsigned int dwBuff, unsigned int nPutBits);
 
    unsigned char * pbOutBuffer;        // 00 : Output buffer
    unsigned int    dwOutSize;          // 04 : Size of output buffer
    unsigned char * pbOutPos;           // 08 : Current output position
    unsigned int    dwBitBuff;          // 0C : Bit buffer
    unsigned int    nBits;              // 10 : Number of bits in the bit buffer
};
...
struct THTreeItem
{
    ...
    THTreeItem * GetPrevItem(int value);
    void         ClearItemLinks();
    void         RemoveItem();
 
    THTreeItem  * next;                 // 00 - Pointer to next THTreeItem
    THTreeItem  * prev;                 // 04 - Pointer to prev THTreeItem (< 0 if none)
    unsigned int dcmpByte;             // 08 - Index of this item in item pointer array, decompressed byte value
    unsigned int byteValue;            // 0C - Some byte value
...
struct TQDecompress
{
    ...
    unsigned int nBits;                // 04 - Bit count
    union
    {
        unsigned int dcmpByte;         // 08 - Byte value for decompress (if bitCount <= 7)
...
//  void  ModifyTree(unsigned int dwIndex);
//  void  UninitTree();
...
//  void  Call15007010(Bit32 dwInLength, THTreeItem * item);
    ...
    unsigned int bIsCmp0;              // 0000 - 1 if compression type 0
    ...
    unsigned int nItems;               // 3068 - Number of used HTree items
