...
THTreeItem * THTreeItem::GetPrevItem(int value)
{
    if(PTR_INVALID(prev))
        return PTR_NOT(prev);
...
static void RemoveItem(THTreeItem * item)
{
    ...
    if(PTR_INT(prev) < 0)
        ...
        prev = (THTreeItem *)((unsigned char *)prev + (unsigned int)((unsigned char *)item - (unsigned char *)(next->prev)));
...
void TOutputStream::PutBits(unsigned int dwBuff, unsigned int nPutBits)
{
...
unsigned int TInputStream::GetBit()
{
    unsigned int dwBit = (dwBitBuff & 1);
 
    dwBitBuff >>= 1;
    if(--nBits == 0)
    {
        dwBitBuff   = LITTLEENDIAN32BITS(*(unsigned int *)pbInBuffer);
...
unsigned int TInputStream::Get7Bits()
{
    if(nBits <= 7)
    {
...
static void InsertItem(THTreeItem ** itemPtr, THTreeItem * item, unsigned int where, THTreeItem * item2)
{
    THTreeItem * next = item->next;     // EDI - next to the first item
    THTreeItem * prev = item->prev;     // ESI - prev to the first item
    int next2;                         // Pointer to the next item
...
//              next2 = (THTreeItem *)(unsigned int)((unsigned char *)item2 - (unsigned char *)(item2->next->prev));
 
//          prev2 = (THTreeItem *)((char *)prev2 + (unsigned int)next2);// ???
            prev2 += next2;
            prev2->next = item;
...
{
    unsigned int   maxByte;            // [ESP+10] - The greatest character found in table
    ...
    THTreeItem    * child1;
    unsigned int   i;                  // egcs in linux doesn't like multiple for loops without an explicit i
...
void THuffmannTree::ModifyTree(unsigned int dwIndex)
{
...
_1500E549:           
            ...
            if(bHasQdEntry == false)
            {
                ...
                {
                    unsigned int nIndex = n7Bits & (0xFFFFFFFF >> (32 - nBitCount));
                    unsigned int nAdd   = (1 << nBitCount);
...
unsigned int THuffmannTree::DoDecompression(unsigned char * pbOutBuffer, unsigned int dwOutLength, TInputStream * is)
{
    ...
    unsigned int bitCount;             // Bit counter if walking HTree
    unsigned int oneByte;              // 8 bits from bit stream/Pointer to target
    ...
    unsigned int dcmpByte = 0;         // Decompressed byte value
    bool          bFlag = 0;
    ...
    for(;;)
    {
        ...
        unsigned int sevenBits = is->Get7Bits();// 7 bits from input stream
        ...
        if(bFlag == 1)
        {
            ...
            if(hasQDEntry == 0)
            {
                ...
                {
                    unsigned int index = sevenBits & (0xFFFFFFFF >> (32 - bitCount)); // Index for quick-decompress entry
                    unsigned int addIndex = (1 << bitCount);                          // Add value for index
    ...
    return (unsigned int)(outPtr - pbOutBuffer);
}
*/
