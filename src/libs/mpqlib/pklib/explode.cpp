...
{
    unsigned int nBits;                // Number of bits to skip
    unsigned int value;                // Position in buffers
    ...
    if(pWork->bit_buff & 1)
    {
        ...
        {
            unsigned int val2 = pWork->bit_buff & ((1 << nBits) - 1);
...
static unsigned int DecodeDist(TDcmpStruct * pWork, unsigned int dwLength)
{
    unsigned int pos   = pWork->position1[(pWork->bit_buff & 0xFF)];
    unsigned int nSkip = pWork->DistBits[pos];     // Number of bits to skip
...
static unsigned int Expand(TDcmpStruct * pWork)
{
    ...
    unsigned int oneByte;              // One byte from compressed file
    ...
    while((dwResult = oneByte = DecodeLit(pWork)) < 0x305)
    {
        ...
        if(oneByte >= 0x100)
        {
            ...
            unsigned int  copyLength = oneByte - 0xFE;
            unsigned int  moveBack;
