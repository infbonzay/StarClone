...
{ \
	...
	{ \
		fprintf(stderr,"libsmacker::smk_huff_safe_build_rec(" #bs "," #p ") - ERROR (file: %s, line: %u)\n", __FILE__, (unsigned int)__LINE__); \
		goto error; \
	} \
}
...
{ \
	...
	{ \
		fprintf(stderr,"libsmacker::smk_huff_big_safe_build_rec(" #bs "," #cache "," #low8 "," #hi8 "," #p ") - ERROR (file: %s, line: %u)\n", __FILE__, (unsigned int)__LINE__); \
		goto error; \
	} \
}
