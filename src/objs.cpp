                              ...
                              int readyatbegin, int persshield, int perslife, int energypers, unsigned short imagelo_id)
{
    ...
    else
    {
        ...
        else if (playernr >= PLAYEDPLAYERS && playernr != GREYNEUTRALCOLORPLAYER)
...
escapeconstrslots:
        ...
        case MODEPOWEROFF:
            ...
            else if (a->SC_Unit == SC_BATTERYOBJ || GetConstructNR(a))
    ...
    if (SC_Unit != SC_NOUNITNR)
    {
        if (!IsHallucination(a))
        {
            ...
            else
            {
                if (!IsBuild(SC_Unit))
                    changesupply = 1;
                else if (a->SC_FromUnit == SC_HIVEOBJ || a->SC_FromUnit == SC_LAIROBJ)	//hive or lair
                    changesupply = 1;
            }
            if (changesupply)
            {
...
int tryaiaction(OBJ *a, OBJ *atacker, int directiondamage)
{
    ...
    if (IsCloakable(a->SC_Unit))
    {
        ...
        else if (accesstomove(a, loadobj(a->SC_Unit), MODEPERSONNELCLOAK, a->playernr))
        {
            //cloakunit
            if (!GetMageAtr(&a->atrobj, ATRINVISIBLE))
            {
