...
{ \
	...
	{ \
		fprintf(stderr,"libsmacker::smk_huff_safe_build(" #bs "," #t ") - ERROR (file: %s, line: %u)\n", __FILE__, (unsigned int)__LINE__); \
		goto error; \
	} \
}
...
{ \
	...
	{ \
		fprintf(stderr,"libsmacker::smk_huff_safe_lookup(" #bs "," #t "," #s ") - ERROR (file: %s, line: %u)\n", __FILE__, (unsigned int)__LINE__); \
		goto error; \
	} \
}
...
{ \
	...
	{ \
		fprintf(stderr,"libsmacker::smk_huff_big_safe_build(" #bs "," #t ") - ERROR (file: %s, line: %u)\n", __FILE__, (unsigned int)__LINE__); \
		goto error; \
	} \
}
...
{ \
	...
	{ \
		fprintf(stderr,"libsmacker::smk_huff_big_safe_lookup(" #bs "," #t "," #s ") - ERROR (file: %s, line: %u)\n", __FILE__, (unsigned int)__LINE__); \
		goto error; \
	} \
}
/* Look up a 16-bit value in the bigtree by following a bitstream
	returns -1 on error */
int smk_huff_big_lookup (struct smk_bit_t *, struct smk_huff_big_t *);
