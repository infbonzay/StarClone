...
BOOL  WINAPI SFILE(OpenArchive)(LPCSTR lpFileName, UINT dwPriority, UINT dwFlags, HANDLE *hMPQ);
...
UINT WINAPI SFILE(GetFileSize)(HANDLE hFile, DWORD *lpFileSizeHigh);
...
BOOL  WINAPI SFILE(ReadFile)(HANDLE hFile,LPVOID lpBuffer,UINT nNumberOfBytesToRead,LPDWORD lpNumberOfBytesRead,LPOVERLAPPED lpOverlapped);
UINT  WINAPI SFILE(SetLocale)(UINT nNewLocale);
BOOL  WINAPI SFILE(GetBasePath)(LPCSTR lpBuffer, UINT dwBufferLength);
